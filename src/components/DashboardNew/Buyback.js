...
const buyback = async () =>
{
    ...
    try {
        ...
        let transaction2 = algosdk.makeAssetTransferTxnWithSuggestedParamsFromObject({
          ...
          console.log("Elem", parseInt(parseFloat(elemAmount).toFixed(6) * 1000000));
      ...
      //   const signedTx2 = await myAlgoWallet.signTransaction(txs[1].toByte());
        const signedTxEscrow = algosdk.signLogicSigTransaction(txs[2], lsig);
...
const buybackWalletCheck = async () =>
{
    if(localStorage.getItem("walletName") === "myAlgoWallet")
    {
        await buyback();
                        ...
                        <div className="d-flex align-items-center float-end mt-1 acc-h-links mb-4">
                            ...
                            <div className="group-row">
                                <Row>
                                    ...
                                    <Col sm={7}>
                                        <div className="input-group-max px-3 input-group-max-lg w-100">
                                            <input type="number" placeholder='0.00' value={(((parseFloat(elemAmount)* elemPrice)/usdcPrice).toFixed(6))} readonly disabled className='form-control' />
                                        </div>
                                    </Col>
                                </Row>
                            </div>
                            ...
                            <ButtonLoad loading={loader} className='btn w-100 btn-blue' onClick={buybackWalletCheck}>
                                Buyback
                            </ButtonLoad>
                        </Card>
                    </Col>
