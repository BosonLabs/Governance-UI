...
const GenesisMarket = () => {
    useEffect(() => {
        document.title = "ELEMENT | GenesisMarket"
    }, [])
    ...
    const filterdata1=()=>{
        ...
        {                          
        if(getrecent === "Recently added"){                    
          let data=getImgreffalgosaleSports.reverse().filter((val)=> (val.TimeStamp) <= weekdate || (val.TimeStamp) >= dateset)
          //  console.log("fspo",data)
          return data;        
        }
        if(getrecent === "Low to High"){
    ...
    const filterdata2=()=>{
        ...
        {                          
        if(getrecent === "Recently added"){        
          let data=getImgreffalgosalePet.reverse().filter((val)=> (val.TimeStamp) <= weekdate || (val.TimeStamp) >= dateset)
        //   console.log("filtercall12",data)
          return data;        
        }
        if(getrecent === "Low to High"){
    ...
    const filterdata3=()=>{
        ...
        {                          
        if(getrecent === "Recently added"){        
          let data=getImgreffalgosaleArts.reverse().filter((val)=> (val.TimeStamp) <= weekdate || (val.TimeStamp) >= dateset)
          console.log("filtercall12",data)
          return data;        
        }
        if(getrecent === "Low to High"){
    ...
    const filterdata4=()=>{
        ...
        let today= new Date();
        let weekdate=new Date(today.getFullYear(), today.getMonth(), today.getDate()-3).toDateString();
        console.log("DateExplorePho",weekdate)
        console.log("DateExplore2Pho",dateset)
        if(searchText === "")
        {                          
        if(getrecent === "Recently added"){        
          let data=getImgreffalgosalePhotography.reverse();
          //.filter((val)=> (val.TimeStamp) <= weekdate || (val.TimeStamp) >= dateset)
        console.log("filtercall12pp",data)
          return data;        
        }
        if(getrecent === "Low to High"){
    ...
    const filterdata5=()=>{
        ...
        {                          
        if(getrecent === "Recently added"){        
          let data=getImgreffalgosaleTradingCards.reverse().filter((val)=> (val.TimeStamp) <= weekdate || (val.TimeStamp) >= dateset)
        //   console.log("filtercall12",data)
          return data;        
        }
        if(getrecent === "Low to High"){
                    ...
                    <Tabs defaultActiveKey="all" id="uncontrolled-tab-example" className="mb-3">
                        <Tab eventKey="all" title="Explore">
                            ...
                            {getImgreffalgosale === null || getImgreffalgosale === "" || getImgreffalgosale === undefined || getImgreffalgosale[0] === null || getImgreffalgosale[0] === "" || getImgreffalgosale[0] === undefined || filterdata()[0] === null || filterdata()[0] === "" || filterdata()[0] === undefined ? (
                                ...
                                <Link to="/genesis-market" className='btn btn-primary'>Browse marketplace</Link>
                                </div>
                            ) : (
                            <>
